...
int make_hist( const char *text, int hist[ALPHA_SIZE] )
{
    ...
    /* Reset the histogram */
#pragma omp parallel for default(none) shared(hist) private(j)
    for (j=0; j<ALPHA_SIZE; j++) {
        hist[j] = 0;
    }

    /* Count occurrences */
#pragma omp parallel default(none) shared(TEXT_LEN, text) private(i) reduction(+:nlet) reduction(+:hist[:ALPHA_SIZE])
    {
//        int priv_hist[ALPHA_SIZE] = {0};
...
#pragma omp for
        for (i=0; i<TEXT_LEN; i++) {
            ...
            if (isalpha(c)) {
                nlet++;
                const size_t idx_hist = tolower(c) - 'a';
                hist[idx_hist]++;
                //priv_hist[my_id][idx_hist]++;
            }
        }
        //hist[?] += priv_hist[?][?];
    }
