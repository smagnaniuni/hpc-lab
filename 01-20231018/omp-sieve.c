...
long mark( char *isprime, int k, long from, long to )
{
    ...
    // 2 3 4 5 6 7 8 9 10
    // 2 -> 4 6 8 10 -> 4 primes (2,3,5,7)
    // 3 -> 9
    //printf("mark(k: %i from/to:[%li-%li]) n: %li\n", k, from, to, n);
#pragma omp parallel default(none) shared(isprime, k, from, to, n) reduction(+:nmarked)
    {
        long partial = 0;
#if 0
#pragma omp for
        for ( long x=from; x<to; x+=k ) {
            printf("thread %li/%li %li\n", my_id, num_threads, x);
//        printf("thread %li/%li start/end:[%li-%li]\n", my_id, num_threads, my_start, my_end);
#else
        const long my_id = omp_get_thread_num();
        const long num_threads = omp_get_num_threads();
        long my_start = from + (n * my_id) / num_threads;
        const long my_end = from + (n * (my_id + 1)) / num_threads;
        my_start = ((my_start + k - 1)/k)*k; /* start from the lowest multiple of p that is >= from */
        for ( long x=my_start; x<my_end; x+=k ) {
            //printf("thread %li/%li start/end:[%li-%li] current %li\n", my_id, num_threads, my_start, my_end, x);
#endif
            if (isprime[x]) {
                isprime[x] = 0;
                //nmarked++;
