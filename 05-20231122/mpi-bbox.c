...
int main( int argc, char* argv[] )
{
    ...
    // Scatter the x1, y1, x2, y2 to the other processes
    const int local_size = N / comm_sz;
    float* local_x1 = (float*)malloc(local_size * sizeof(float));   assert(local_x1 != NULL);
    float* local_y1 = (float*)malloc(local_size * sizeof(float));   assert(local_y1 != NULL);
    float* local_x2 = (float*)malloc(local_size * sizeof(float));   assert(local_x2 != NULL);
    float* local_y2 = (float*)malloc(local_size * sizeof(float));   assert(local_y2 != NULL);
    MPI_Scatter(x1, local_size, MPI_FLOAT, local_x1, local_size, MPI_FLOAT, ROOT, MPI_COMM_WORLD);
    MPI_Scatter(y1, local_size, MPI_FLOAT, local_y1, local_size, MPI_FLOAT, ROOT, MPI_COMM_WORLD);
    MPI_Scatter(x2, local_size, MPI_FLOAT, local_x2, local_size, MPI_FLOAT, ROOT, MPI_COMM_WORLD);
    MPI_Scatter(y2, local_size, MPI_FLOAT, local_y2, local_size, MPI_FLOAT, ROOT, MPI_COMM_WORLD);

    // Each process computes the bbox
    bbox(local_x1, local_y1, local_x2, local_y2, local_size, &xb1, &yb1, &xb2, &yb2);

    float res_xb1, res_yb1, res_xb2, res_yb2;
    // Reduce
    MPI_Reduce(&xb1, &res_xb1, 1, MPI_FLOAT, MPI_MIN, ROOT, MPI_COMM_WORLD);
    MPI_Reduce(&yb1, &res_yb1, 1, MPI_FLOAT, MPI_MAX, ROOT, MPI_COMM_WORLD);
    MPI_Reduce(&xb2, &res_xb2, 1, MPI_FLOAT, MPI_MAX, ROOT, MPI_COMM_WORLD);
    MPI_Reduce(&yb2, &res_yb2, 1, MPI_FLOAT, MPI_MIN, ROOT, MPI_COMM_WORLD);

    if(my_rank == ROOT) {
        /* Print bounding box */
        printf("bbox: %f %f %f %f\n", res_xb1, res_yb1, res_xb2, res_yb2);
    }
