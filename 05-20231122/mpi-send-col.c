...
int main( int argc, char *argv[] )
{
    ...
    // First send right column to leftmost ghost column of next rank
    const int HALO = 1;
    const int LOCAL_LEFT_GHOST = 0;
    const int LOCAL_LEFT = LOCAL_LEFT_GHOST + HALO;
    const int LOCAL_RIGHT = LOCAL_LEFT + SIZE - 1;
    ...
    MPI_Datatype MPI_TYPE_COLUMN;
    MPI_Type_vector(SIZE, 1, SIZE + 2 * HALO, MPI_INT, &MPI_TYPE_COLUMN);
    MPI_Type_commit(&MPI_TYPE_COLUMN);

    MPI_Sendrecv(&my_mat[0][LOCAL_RIGHT], 1, MPI_TYPE_COLUMN, rank_next, 0,
                 &my_mat[0][LOCAL_LEFT_GHOST], 1, MPI_TYPE_COLUMN, rank_prev, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    // Send left column to right ghost column of previous rank
    MPI_Sendrecv(&my_mat[0][LOCAL_LEFT], 1, MPI_TYPE_COLUMN, rank_prev, 0,
                 &my_mat[0][LOCAL_RIGHT_GHOST], 1, MPI_TYPE_COLUMN, rank_next, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
