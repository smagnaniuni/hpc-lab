...
void step(cell_t *cur, cell_t *next, int ext_width, int ext_height)
{
    ...
    if (i <= BOTTOM && j <= RIGHT) {
            int nblack = 0;
// loop unroll directive for nvcc
#pragma unroll
...
int main( int argc, char* argv[] )
{
    ...
    const dim3 stepGrid((width + BLKDIM - 1) / BLKDIM,
                        (height + BLKDIM - 1) / BLKDIM); // arrotondati per eccesso

    const dim3 copyLRBlock(1024);
    const dim3 copyLRGrid((ext_height + 1024 - 1) / 1024);
    const dim3 copyTBBlock(1024);
    const dim3 copyTBGrid((ext_width + 1024 - 1) / 1024);
