...
__device__
__host__
unsigned int mtrx_idx(unsigned int i, unsigned int j, int n)
{
    return i * n + j;
}

__global__
void matsum_kernel( float *p, float *q, float *r, int n )
{
    unsigned int i = threadIdx.y + blockIdx.y * blockDim.y;
    unsigned int j = threadIdx.x + blockIdx.x * blockDim.x;
    if(i < n && j < n) {
        unsigned int idx = mtrx_idx(i, j, n);
        r[idx] = p[idx] + q[idx];
    }
}
...
void matsum( float *p, float *q, float *r, int n )
{
    ...
    */
    ...
    const dim3 grid((n + BLKDIM - 1) / BLKDIM, (n + BLKDIM - 1) / BLKDIM);
