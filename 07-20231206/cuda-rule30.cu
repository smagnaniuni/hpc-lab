...
__device__
__host__
cell_t calc_cell(const cell_t left, const cell_t center, const cell_t right)
{
    return
        (left && !center && !right) ||
        (!left && !center && right) ||
...
__global__
void fill_ghost(cell_t* cur, const int LEFT, const int RIGHT, const int LEFT_GHOST, const int RIGHT_GHOST)
{
...
__global__
void step_kernel(cell_t* cur, cell_t* next, int ext_n)
{
    const int LEFT = 1;
    ...
    if (i >= LEFT && i <= RIGHT) {
        ...
        next[i] = calc_cell(left, center, right);
    }
}
...
int main( int argc, char* argv[] )
{
    ...
    const dim3 grid((ext_width + BLKDIM - 1) / BLKDIM);
    ...
    for (s=0; s<steps; s++) {
        ...
        // cur[RIGHT_GHOST] = cur[LEFT];
        // cur[LEFT_GHOST] = cur[RIGHT];
        fill_ghost<<<1, 1>>>(d_cur, LEFT, RIGHT, LEFT_GHOST, RIGHT_GHOST);

        /* Compute next state */
        // step(cur, next, ext_width);
        step_kernel<<<grid, block>>>(d_cur, d_next, ext_width);
