...
int main( int argc, char *argv[] )
{
    ...
    int v = 0;
    if (my_rank == 0) {
        MPI_Send(&v, 1, MPI_INT, (my_rank+1) % comm_sz, 0, MPI_COMM_WORLD);
    }
    // const int recv_from = my_rank == 0 ? comm_sz-1 : (my_rank - 1) % comm_sz;
    const int recv_from = (my_rank - 1 + comm_sz) % comm_sz;
    const int send_to = (my_rank + 1) % comm_sz;
    while ((v/comm_sz) < K) {
        MPI_Recv(&v, 1, MPI_INT, recv_from, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        printf("Proc %i received value %i from %i\n", my_rank, v, recv_from);
        v++;
        printf("Proc %i sending value %i to %i\n", my_rank, v, send_to);
        MPI_Send(&v, 1, MPI_INT, send_to, 0, MPI_COMM_WORLD);
    }
