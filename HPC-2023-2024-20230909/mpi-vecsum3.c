...
int main( int argc, char* argv[] )
{
    ...
    /* Ever process computes sendcounts and displacements; actually,
       only the master needs this information so we are wasting some
       memory on each MPI node, but this makes the program somewhat
       simpler. */
    ...
    /* All nodes (including the master) allocate the local vectors.
       Note that local_n may be different from each node, if n is not
       a multiple of comm_sz. */
