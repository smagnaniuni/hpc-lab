...
/* if *a > *b, swap them. Otherwise do nothing */
__device__
void cmp_and_swap_kernel( int* a, int* b )
{
    if ( *a > *b ) {
        int tmp = *a;
        *a = *b;
...
__global__
void odd_even_step_bad( int *x, int n, int phase )
{
    const int idx = threadIdx.x + blockIdx.x * blockDim.x;
    if ( (idx < n-1) && ((idx % 2) == (phase % 2)) ) {
        cmp_and_swap_kernel(&x[idx], &x[idx+1]);
    }
}

__global__
void odd_even_step_kernel(int* x, int n, int phase)
{
    const int nhalfupper = (n + 1) / 2; // TODO VERIFY
...
{
    int* d_v;
    const size_t SIZE_V = n * sizeof(int);
    cudaSafeCall(cudaMalloc((void**)&d_v, SIZE_V));
    cudaSafeCall(cudaMemcpy(d_v, v, SIZE_V, cudaMemcpyHostToDevice));
    ...
    for (int phase = 0; phase < n; phase++) {
        ...
        // odd_even_step_bad<<<1, BLKDIM>>>(d_v, n, phase);
        odd_even_step_kernel<<<1, BLKDIM>>>(d_v, n, phase);
        cudaCheckError();
    }

    cudaSafeCall(cudaMemcpy(v, d_v, SIZE_V, cudaMemcpyDeviceToHost));
