...
void do_static(const int *vin, int *vout, int n)
{
    ...
    */
    ...
    printf("do_static n: %i chunk_size: %li\n", n, chunk_size);
#pragma omp parallel default(none) shared(vin, vout, n, chunk_size)
    {
        ...
        const long START = my_id * chunk_size;
        const long STRIDE = num_threads * chunk_size;
        //printf("[%li/%li] START %li STRIDE %li\n", my_id, num_threads, START, STRIDE);
        for (size_t i=START; i<n; i+=STRIDE) {
            for (size_t j=i; j<i+chunk_size && j<n; j++) {
                //printf("[%li/%li] %lu %lu\n", my_id, num_threads, i, j);
                vout[i] = fib_rec(vin[i]);
            }
        }
    }
...
void do_dynamic(const int *vin, int *vout, int n)
{
    ...
    */
    ...
    size_t idx = 0;
    const long chunk_size = 1;
#pragma omp parallel default(none) shared(vin, vout, n, idx, chunk_size)
    {
        long my_idx;
        do {
#pragma omp critical
            ...
            for (size_t i=my_idx; i<my_idx + chunk_size && i<n; i++) {
                vout[i] = fib_rec(vin[i]);
            }
        } while(my_idx < n);
    }
