...
void cat_map( PGM_image* img, int k )
{
    ...
    /*
     * N is a constant
     * there is a dependency between the inner loops and the outer loop where the buffer swap happens
     * we parallelize the inner loops
     */
    for (i=0; i<k; i++) {
#pragma omp parallel for default(none) collapse(2) shared(next, cur, N)
        for (y=0; y<N; y++) {
            for (x=0; x<N; x++) {
                const int xnext = (2*x+y) % N; //S1
                const int ynext = (x + y) % N; //S2
                next[ynext*N + xnext] = cur[x+y*N]; //S3
            }
        }
        /* Swap old and new */
        tmp = cur; //S4
...
void cat_map_interchange( PGM_image* img, int k )
{
    ...
    /* [TODO] Which of the following loop(s) can be parallelized? */
    /*
     * we parallelize the outer loops
     */
#pragma omp parallel for default(none) collapse(2) shared(next, cur, N, k) private(i)
    for (y=0; y<N; y++) {
        for (x=0; x<N; x++) {
